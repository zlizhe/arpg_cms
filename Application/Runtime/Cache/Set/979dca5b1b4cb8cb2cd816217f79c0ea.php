...
<?php if(!$slideArr): ?><blockquote>没有找到信息</blockquote>
	
<?php else: ?>
	
	<form role="form" method="post" target="actionfrm" action="<?php echo U('Set/Ajax/setEditAllSlide');?>" enctype="multipart/form-data">
	...
	<table class="table table-hover">
		...
		<tbody>
			<?php if(is_array($slideArr)): foreach($slideArr as $key=>$value): ?><tr>
					...
					<td>
					    <input type="text" class="form-control input-mini" value="<?php echo ($value['link']); ?>" name="link[<?php echo ($value['id']); ?>]" />
					</td>
					<td class="text-center">
						<input type="checkbox" name="target[<?php echo ($value['id']); ?>]" value="1" <?php if($value['target']): ?>checked<?php endif; ?>>
					</td>
				</tr>
...
<form action="<?php echo U('/set/ajax/uploadSlide');?>" class="form-horizontal hide" method="post" target="actionfrm" id="cover_form" name="cover_form" enctype="multipart/form-data">
