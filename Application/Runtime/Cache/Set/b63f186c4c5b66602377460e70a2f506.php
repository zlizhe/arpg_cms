...
<?php else: ?>
	<blockquote>总共找到 <?php echo ($articleArr['how']); ?> 条信息
	<?php if($query): ?><small>
		<span class="label label-primary">
		关键词: "<?php echo ($query); ?>" <a href="<?php echo U($urlName, array('query'=>NULL));?>" title="清除" class="del"><span class="glyphicon glyphicon-remove"></span></a> 
		</span>
		</small><?php endif; ?>
	<?php if($cat_id): ?><small>
		<span class="label label-primary">
		分类ID: "<?php echo ($cat_id); ?>" <a href="<?php echo U($urlName, array('cat_id'=>NULL));?>" title="清除" class="del"><span class="glyphicon glyphicon-remove"></span></a> 
		</span>
		</small><?php endif; ?>
	...
	<table class="table table-hover">
		...
		<tbody>
			<?php if(is_array($articleArr['row'])): foreach($articleArr['row'] as $key=>$value): ?><tr>
					<td>
					<?php if($app == 1): ?><input type="checkbox" name="arpg_check" id="checkOn" value="<?php echo ($value[id]); ?>">
					<?php else: ?>
						<?php echo ($value['id']); endif; ?>
					</td>

					<td>
						<a href="/article/<?php echo ($value['id']); ?>/<?php echo ($value['title']); ?>" target="_blank">
						<?php echo ($value['title']); ?>
						<?php if($value['cover_img']): ?><span class="glyphicon glyphicon-picture"></span><?php endif; ?>
						</a>
					</td>
					<td><a href="<?php echo U($urlName, array('cat_id'=>$value['cat_id'],'query'=>NULL));?>"><?php echo ($value['cat_name']); ?></a></td>
					...
					<td class="dropdown">
						...
						<ul class="dropdown-menu" role="menu" aria-labelledby="comment_usercat">
							<li role="presentation"><a role="menuitem" tabindex="-1" href="<?php echo U($urlName, array('created_uid'=>$value['created_uid'],'query'=>NULL));?>">只显示该用户发布</a></li>
