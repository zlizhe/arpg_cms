		...
		<article>
			<ul class="media-list">
				<?php if(is_array($articleArr['row'])): foreach($articleArr['row'] as $key=>$value): ?><li class="media">
	                	<?php if($value['cover_img']): ?><a class="pull-left" href="/article/<?php echo ($value['id']); ?>/<?php echo ($value['title']); ?>">
		                        <img class="media-object img-rounded" src="<?php echo ($value['cover_img']); ?>" />
		                    </a><?php endif; ?>
	                    <div class="media-body">
	                        <h3 class="media-heading">
	                            <a href="/article/<?php echo ($value['id']); ?>/<?php echo ($value['title']); ?>"><?php echo ($value['title']); ?></a>
	                        </h3>
							...
							<p class="summary">
								<a href="/article/<?php echo ($value['id']); ?>/<?php echo ($value['title']); ?>">
						...
						<?php if($value['tags_arr']): ?><div class="pull-right">
								<ul class="list-unstyled list-inline tags">
									<?php if(is_array($value['tags_arr'])): foreach($value['tags_arr'] as $key=>$value2): ?><li><a href="<?php echo U($urlName);?>/query/<?php echo ($value2); ?>"><?php echo ($value2); ?></a></li><?php endforeach; endif; ?>
								</ul>
		                    </div><?php endif; ?>
